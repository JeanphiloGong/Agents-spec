# Reading Order And Navigation Map

Use this reference when the repository needs explicit reading routes rather
than only correct file placement.

## Core Reading Route

Most healthy documentation systems expose a route similar to:
- entrypoint
- overview
- authority page
- local detail

## Reading Paths By Intent

Common intent-based routes:
- new contributor: root entrypoint -> system overview -> module overview ->
  local current-state
- implementer: module entrypoint -> local current-state -> detailed plan ->
  verification notes
- reviewer: overview -> authority page -> related detail or historical context

## Rule

A doc tree is not enough. At least one explicit navigation path should exist
for the primary reader intents.
